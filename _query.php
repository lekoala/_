...
class _query
{
    ...
    function yield_all($fetch_type = null, $fetch_argument = null)
    {
        if ($this->fetch_class && $fetch_type == null) {
            $fetch_type = PDO::FETCH_CLASS;
            $fetch_argument = $this->fetch_class;
        ...
        $results = $this->query();
        if ($results) {
            while ($row = $results->fetch($fetch_type)) {
                yield $row;
            }
        }
        return array();
    ...
    protected function get_pdo()
    {
        ...
        $pdo = _::$pdo;
        if ($pdo === null) {
            throw new Exception('You must define _::$pdo to use the query builder');
        }
        self::$pdo = $pdo;
        return $pdo;
    }
